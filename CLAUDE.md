...
`dlLoad()` (shared/site.js:2250) merges the baked (published) data with the local working copy: local entries win by `id`, and baked entries the browser doesn't know about are kept so a cleared `localStorage` never drops live content.

### Publishing = DOM snapshot

`buildPublishHTML()` (shared/site.js:1974) clones the **live `document.documentElement`** and serializes it. Consequences to keep in mind:
...
- `#dl-wrap` is stripped (shared/site.js:845): the dev log renders from data on load, so baking it would produce two copies. Same reason it's removed before re-mounting in `dlMountLog()` (shared/site.js:2738), and excluded from the editable-field walk in `csFields()` (:842) and `migrateCaseSnapshot()` (:1114).
- Private entries are never baked (`dlPageData` → `dlPublicData`, shared/site.js:3336 → :3343).
...
- Entry `id` doubles as its URL slug (`#/dev/<id>`) and follows the title until the entry is published, then freezes (`dlRetitle`, shared/site.js:2214) so shared links don't break.
- Adding a block type touches four places: `DE_BLOCKS` (shared/site.js:3022) + `DE_NEW` (:3040) for the palette and default shape, `deRenderBlocks()` (shared/site.js:3227) for the editor UI, `dlRenderBlock()` (shared/site.js:2579) for the public render. If it holds media, also the ref-walkers `dlMediaRefs()` (shared/site.js:2480) and `dlExportData()` (shared/site.js:3354), which scan `b.src`, `b.a`, `b.b` and `b.items`.
- Changing entry shape means bumping `DL_SCHEMA` (currently `3`, shared/site.js:2289) and extending `dlMigrate()` (shared/site.js:2291). Migration is idempotent, gated on `e.v`, and runs on every load — old published entries are upgraded in place.
- Routing: project pages handle `#/dev/<id>`; the landing page only redirects legacy `#/<project>/dev/<id>` links (`dlRoute`/`dlLegacyRedirect`, shared/site.js:2778 / :2785).
- Media resolution order is `MEDIA_CACHE` (unpublished, in memory) → `ref.data` (export) → `asset(path)`, in `dlSrc()` (shared/site.js:2404). On error, `dlMediaFallback()` retries from IndexedDB then hides the figure — this is what covers the ~1 min GitHub Pages deploy lag after a publish.

### Theme

`PRESETS` (shared/site.js:1793) hold ~8 seed colours + two font names; `computeVars()` (shared/site.js:1838) derives the full CSS-variable set (shades, `rgba()` tones, dark-mode inversion via luminance) and `applyTheme()` writes them onto `:root`. `portfolio.css` and `shared/theme.css` read the variables — don't hardcode colours in new CSS (R7).

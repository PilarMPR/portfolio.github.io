...
bash docs/smoke.sh            # loads all eight pages in a real engine; "ok 8 pages" = clear
...
- **R2 — Every `on*=` handler name must resolve to a declaration in `site.js`.** Handlers are wired by string, in the eight HTML files *and* inside `site.js` template literals. Nothing else in the repo can catch a rename that misses one — no import fails, no lint fires; the button simply does nothing when clicked.
- **R3 — Every page keeps the sentinel elements.** `#cursor`, `#nav`, `#nav-logo` and `#lightbox` are captured at load with no null check; removing one throws and kills the rest of the script. `checks.sh` requires all four. (This is why export leaves them in place, inert.) `#fmt-bar` and `#de-toast` were on this list until the chrome moved into script — they are built on demand now, so `fmtBarEl()` (shared/site.js:1167) and `dlToast()` (shared/site.js:2301) must keep tolerating their absence rather than assuming a page has them.
...
- **R7 — No hex literals in new CSS.** Use the variables `computeVars()` writes. 63 literals already survive in `portfolio.css` (against 423 `var()` uses), plus 15 in `shared/theme.css`. The public layer no longer holds a single one: all 63 are the `:root` fallback block and the editor's dark tool UI, which ignores presets by design. Don't add another. These counts drift when the editor republishes the theme — one did on 2026-08-08 — so `checks.sh` recomputes all three and fails if this sentence disagrees with the files.
...
- **R11 — Every `shared/site.js:NNN` cited in this file must still name what it points at.** The browser rewrites `site.js` on every publish, so a citation rots the moment anything above it grows — one batch of commits on 2026-08-07 invalidated 18 of 19 at once, silently. `checks.sh` re-resolves all 42 against the file. Cite a symbol in backticks on the same line as its number, or the check has nothing to match. `docs/backlog.md` is exempt: it pins its numbers to a stated commit.
- **R12 — `site.js` must parse.** One flat script, no module graph, no bundler — a stray brace blanks all eight pages and no other check here would notice. `checks.sh` compiles it without executing it, so browser globals don't matter.
...
`shared/site.js`, `shared/portfolio.css` and `shared/theme.css` are shared by all eight pages. `site.js` is one flat script in global scope (no modules, no bundler) and is wired to its markup through inline `onclick="…"` handlers — so renaming a function means grepping the eight HTML files **and the chrome templates inside `site.js` itself**, which is where most handlers now live, then running `checks.sh` (R2).

### Two card grids on the landing page

`#work > .projects` holds the four authored game cards. `#ai-work > #ai-projects` holds the **Independent AI projects** category, and it now holds three authored cards too — written by hand at the user's direction on 2026-08-11, not editor-added. Both grids link to case studies under `projects/`; nothing in either one points off-site any more. The repo link a visitor used to get from the card lives in the case study's sidebar instead, as a `.sh-repo` block.

CSS still hides `#ai-work` while it is genuinely empty, so an unfilled category never publishes as a heading standing over nothing, and `body.editing` always shows it so the first card can be added. The rule is written `#ai-work:not(:has(.project)) { display:none }` rather than a `display:none` default plus a `:has()` override, so it **fails open** — an engine without `:has()` shows a section with content rather than hiding it, which matters now that the nav links to it.

**`document.querySelector('.projects')` is no longer unambiguous** — it returns the Work grid, which is correct only as a fallback. `insertProjectCard()` (shared/site.js:1254) takes the target grid's id and numbers cards per grid, so both categories start at 01. `autoSave()` records each card's grid by walking up from the card (shared/site.js:1509), and `loadSaved()` (shared/site.js:1567) routes it back there; a `__custom_cards__` record with no `grid` predates this section and falls back to Work.

**A generated card's destination is a field, not an attribute.** `data-ed` round-trips `innerHTML`, so it cannot reach an `href` — the URL lives in a `.proj-link` field that CSS shows only under `body.editing`, and `syncCardLinks()` (shared/site.js:1312) copies it onto the anchor. That function is **the only thing that writes a `.project` href**, and it must run *before* `autoSave()` serializes `outerHTML` or the link dies on the next reload; it is called from insert, from save, and after restore. A blank field removes the attribute entirely rather than leaving a dead `#`. Because cards are anchors, a document-level capture listener swallows clicks on `.project` while editing — excluding `.img-upload-btn` and `.card-del-btn`, which need their own default behaviour. Authored cards have no link field and `syncCardLinks()` skips them.
...
`dlLoad()` (shared/site.js:2326) merges the baked (published) data with the local working copy: local entries win by `id`, and baked entries the browser doesn't know about are kept so a cleared `localStorage` never drops live content.

### Publishing = DOM snapshot

`buildPublishHTML()` (shared/site.js:2050) clones the **live `document.documentElement`** and serializes it. Consequences to keep in mind:
...
- `#dl-wrap` is stripped (shared/site.js:854): the dev log renders from data on load, so baking it would produce two copies. Same reason it's removed before re-mounting in `dlMountLog()` (shared/site.js:2814), and excluded from the editable-field walk in `csFields()` (:851) and `migrateCaseSnapshot()` (:1123).
- Private entries are never baked (`dlPageData` → `dlPublicData`, shared/site.js:3412 → :3419).

`ghPublish()` (shared/site.js:2148) does one commit via the git-data API: blobs for pending media + the page, one tree on top of `HEAD`, one commit, `PATCH` the branch ref. Target repo is hardcoded in `GH` (shared/site.js:2022). Media is committed as **real files under `assets/`** — nothing is base64'd into the page.

`buildExportHTML()` (shared/site.js:1672) is the opposite: inlines CSS/JS and every `assets/` reference as data URIs, and rewrites sibling `.html` links to the live site so a single downloaded file works offline. It no longer has to strip the editor — every publish already did — so what its `forExport` branch removes is only the affordances that live inside page content (upload buttons, `contenteditable`), plus the `data-readonly` flag that makes `toggleEdit()` refuse.
...
- Entry `id` doubles as its URL slug (`#/dev/<id>`) and follows the title until the entry is published, then freezes (`dlRetitle`, shared/site.js:2285) so shared links don't break.
- Adding a block type touches four places: `DE_BLOCKS` (shared/site.js:3098) + `DE_NEW` (:3116) for the palette and default shape, `deRenderBlocks()` (shared/site.js:3303) for the editor UI, `dlRenderBlock()` (shared/site.js:2655) for the public render. If it holds media, also the ref-walkers `dlMediaRefs()` (shared/site.js:2556) and `dlExportData()` (shared/site.js:3430), which scan `b.src`, `b.a`, `b.b` and `b.items`.
- Changing entry shape means bumping `DL_SCHEMA` (currently `3`, shared/site.js:2365) and extending `dlMigrate()` (shared/site.js:2367). Migration is idempotent, gated on `e.v`, and runs on every load — old published entries are upgraded in place.
- Routing: project pages handle `#/dev/<id>`; the landing page only redirects legacy `#/<project>/dev/<id>` links (`dlRoute`/`dlLegacyRedirect`, shared/site.js:2854 / :2861).
- Media resolution order is `MEDIA_CACHE` (unpublished, in memory) → `ref.data` (export) → `asset(path)`, in `dlSrc()` (shared/site.js:2480). On error, `dlMediaFallback()` retries from IndexedDB then hides the figure — this is what covers the ~1 min GitHub Pages deploy lag after a publish.

### Theme

`PRESETS` (shared/site.js:1856) hold ~8 seed colours + two font names — `Editorial` is the default and the one the site ships in: warm paper, one pine-green `accent`, Newsreader for display and Source Sans 3 for text, both linked from every page's `<head>` with their italic axis (which the runtime loader cannot request, so `_loadedFonts` seeds them). `Plain` — white, no accent, `System` faces — stays as a preset. `System` is a font name in the picker but not a webfont: `fontStack()` (shared/site.js:1890) maps it to the device UI stack and `ensureFont()` skips it, so a default page fetches no font at all. `computeVars()` (shared/site.js:1914) derives the full CSS-variable set (shades, `rgba()` tones, dark-mode inversion via luminance) and `applyTheme()` writes them onto `:root`. `portfolio.css` and `shared/theme.css` read the variables — don't hardcode colours in new CSS (R7).

...
`dlLoad()` (shared/site.js:2321) merges the baked (published) data with the local working copy: local entries win by `id`, and baked entries the browser doesn't know about are kept so a cleared `localStorage` never drops live content.

### Publishing = DOM snapshot

`buildPublishHTML()` (shared/site.js:2045) clones the **live `document.documentElement`** and serializes it. Consequences to keep in mind:
...
- `#dl-wrap` is stripped (shared/site.js:854): the dev log renders from data on load, so baking it would produce two copies. Same reason it's removed before re-mounting in `dlMountLog()` (shared/site.js:2809), and excluded from the editable-field walk in `csFields()` (:851) and `migrateCaseSnapshot()` (:1123).
- Private entries are never baked (`dlPageData` → `dlPublicData`, shared/site.js:3407 → :3414).

`ghPublish()` (shared/site.js:2143) does one commit via the git-data API: blobs for pending media + the page, one tree on top of `HEAD`, one commit, `PATCH` the branch ref. Target repo is hardcoded in `GH` (shared/site.js:2017). Media is committed as **real files under `assets/`** — nothing is base64'd into the page.
...
- Adding a block type touches four places: `DE_BLOCKS` (shared/site.js:3093) + `DE_NEW` (:3111) for the palette and default shape, `deRenderBlocks()` (shared/site.js:3298) for the editor UI, `dlRenderBlock()` (shared/site.js:2650) for the public render. If it holds media, also the ref-walkers `dlMediaRefs()` (shared/site.js:2551) and `dlExportData()` (shared/site.js:3425), which scan `b.src`, `b.a`, `b.b` and `b.items`.
- Changing entry shape means bumping `DL_SCHEMA` (currently `3`, shared/site.js:2360) and extending `dlMigrate()` (shared/site.js:2362). Migration is idempotent, gated on `e.v`, and runs on every load — old published entries are upgraded in place.
- Routing: project pages handle `#/dev/<id>`; the landing page only redirects legacy `#/<project>/dev/<id>` links (`dlRoute`/`dlLegacyRedirect`, shared/site.js:2849 / :2856).
- Media resolution order is `MEDIA_CACHE` (unpublished, in memory) → `ref.data` (export) → `asset(path)`, in `dlSrc()` (shared/site.js:2475). On error, `dlMediaFallback()` retries from IndexedDB then hides the figure — this is what covers the ~1 min GitHub Pages deploy lag after a publish.

### Theme

`PRESETS` (shared/site.js:1856) hold ~8 seed colours + two font names — `Plain` is the default and the one the site ships in: white paper, `accent` equal to `text`, and `System` for both faces. `System` is a font name in the picker but not a webfont: `fontStack()` (shared/site.js:1890) maps it to the device UI stack and `ensureFont()` skips it, so a default page fetches no font at all. `computeVars()` (shared/site.js:1909) derives the full CSS-variable set (shades, `rgba()` tones, dark-mode inversion via luminance) and `applyTheme()` writes them onto `:root`. `portfolio.css` and `shared/theme.css` read the variables — don't hardcode colours in new CSS (R7).

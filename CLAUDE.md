...
- **R2 — Every `on*=` handler name must resolve to a declaration in `site.js`.** Handlers are wired by string, in the nine HTML files *and* inside `site.js` template literals. Nothing else in the repo can catch a rename that misses one — no import fails, no lint fires; the button simply does nothing when clicked.
- **R3 — Every page keeps the sentinel elements.** `#cursor`, `#nav`, `#nav-logo` and `#lightbox` are captured at load with no null check; removing one throws and kills the rest of the script. `checks.sh` requires all four. (This is why export leaves them in place, inert.) `#fmt-bar` and `#de-toast` were on this list until the chrome moved into script — they are built on demand now, so `fmtBarEl()` (shared/site.js:1168) and `dlToast()` (shared/site.js:2289) must keep tolerating their absence rather than assuming a page has them.
...
- **R12 — `site.js` must parse.** One flat script, no module graph, no bundler — a stray brace blanks all nine pages and no other check here would notice. `checks.sh` compiles it without executing it, so browser globals don't matter.
...
`shared/site.js`, `shared/portfolio.css` and `shared/theme.css` are shared by all nine pages. `site.js` is one flat script in global scope (no modules, no bundler) and is wired to its markup through inline `onclick="…"` handlers — so renaming a function means grepping the nine HTML files **and the chrome templates inside `site.js` itself**, which is where most handlers now live, then running `checks.sh` (R2).

### Two card grids on the landing page

`#work > .projects` holds the four authored game cards. `#ai-work > #ai-projects` holds the **Independent AI projects** category, and it now holds four authored cards too — written by hand at the user's direction on 2026-08-11, not editor-added. Both grids link to case studies under `projects/`; nothing in either one points off-site any more. The repo link a visitor used to get from the card lives in the case study's sidebar instead, as a `.sh-repo` block.
...
`dlLoad()` (shared/site.js:2314) merges the baked (published) data with the local working copy: local entries win by `id`, and baked entries the browser doesn't know about are kept so a cleared `localStorage` never drops live content.

### Publishing = DOM snapshot

`buildPublishHTML()` (shared/site.js:2038) clones the **live `document.documentElement`** and serializes it. Consequences to keep in mind:
...
- `#dl-wrap` is stripped (shared/site.js:855): the dev log renders from data on load, so baking it would produce two copies. Same reason it's removed before re-mounting in `dlMountLog()` (shared/site.js:2802), and excluded from the editable-field walk in `csFields()` (:852) and `migrateCaseSnapshot()` (:1124).
- Private entries are never baked (`dlPageData` → `dlPublicData`, shared/site.js:3400 → :3407).

`ghPublish()` (shared/site.js:2136) does one commit via the git-data API: blobs for pending media + the page, one tree on top of `HEAD`, one commit, `PATCH` the branch ref. Target repo is hardcoded in `GH` (shared/site.js:2010). Media is committed as **real files under `assets/`** — nothing is base64'd into the page.

`buildExportHTML()` (shared/site.js:1673) is the opposite: inlines CSS/JS and every `assets/` reference as data URIs, and rewrites sibling `.html` links to the live site so a single downloaded file works offline. It no longer has to strip the editor — every publish already did — so what its `forExport` branch removes is only the affordances that live inside page content (upload buttons, `contenteditable`), plus the `data-readonly` flag that makes `toggleEdit()` refuse.
...
- Entry `id` doubles as its URL slug (`#/dev/<id>`) and follows the title until the entry is published, then freezes (`dlRetitle`, shared/site.js:2278) so shared links don't break.
- Adding a block type touches four places: `DE_BLOCKS` (shared/site.js:3086) + `DE_NEW` (:3104) for the palette and default shape, `deRenderBlocks()` (shared/site.js:3291) for the editor UI, `dlRenderBlock()` (shared/site.js:2643) for the public render. If it holds media, also the ref-walkers `dlMediaRefs()` (shared/site.js:2544) and `dlExportData()` (shared/site.js:3418), which scan `b.src`, `b.a`, `b.b` and `b.items`.
- Changing entry shape means bumping `DL_SCHEMA` (currently `3`, shared/site.js:2353) and extending `dlMigrate()` (shared/site.js:2355). Migration is idempotent, gated on `e.v`, and runs on every load — old published entries are upgraded in place.
- Routing: project pages handle `#/dev/<id>`; the landing page only redirects legacy `#/<project>/dev/<id>` links (`dlRoute`/`dlLegacyRedirect`, shared/site.js:2842 / :2843).
- Media resolution order is `MEDIA_CACHE` (unpublished, in memory) → `ref.data` (export) → `asset(path)`, in `dlSrc()` (shared/site.js:2468). On error, `dlMediaFallback()` retries from IndexedDB then hides the figure — this is what covers the ~1 min GitHub Pages deploy lag after a publish.

### Theme

`PRESETS` (shared/site.js:1857) hold ~8 seed colours + two font names; `computeVars()` (shared/site.js:1902) derives the full CSS-variable set (shades, `rgba()` tones, dark-mode inversion via luminance) and `applyTheme()` writes them onto `:root`. `portfolio.css` and `shared/theme.css` read the variables — don't hardcode colours in new CSS (R7).

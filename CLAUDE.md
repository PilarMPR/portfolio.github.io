...
- **R4 — New editor UI joins the scrub list** in `buildPublishHTML()` (shared/site.js:1660), or it is serialized into every future publish. The stray `#ep-section-list` / `#ep-presets` markup in `projects/block-city.html` is what that looks like when it's missed.
...
- **R6 — No new silent catches.** `catch (e) {}` is why `localStorage` quota failures are indistinguishable from successful saves (shared/site.js:763, 1086, 1106, 1137, 1231). Every new catch reaches the user via `dlToast()` or `alert()`, or at minimum `console.warn`.
...
`dlLoad()` (shared/site.js:1923) merges the baked (published) data with the local working copy: local entries win by `id`, and baked entries the browser doesn't know about are kept so a cleared `localStorage` never drops live content.

### Publishing = DOM snapshot

`buildPublishHTML()` (shared/site.js:1660) clones the **live `document.documentElement`** and serializes it. Consequences to keep in mind:
...
- `#dl-wrap` is stripped (shared/site.js:1666): the dev log renders from data on load, so baking it would produce two copies. Same reason it's removed before re-mounting in `dlMountLog()` (shared/site.js:2392), and excluded from the editable-field walk in `csFields()` (:535) and `migrateCaseSnapshot()` (:812).
- Private entries are never baked (`dlPageData` → `dlPublicData`, shared/site.js:2987 → :2994).

`ghPublish()` (shared/site.js:1769) does one commit via the git-data API: blobs for pending media + the page, one tree on top of `HEAD`, one commit, `PATCH` the branch ref. Target repo is hardcoded in `GH` (shared/site.js:1632). Media is committed as **real files under `assets/`** — nothing is base64'd into the page.

`buildExportHTML()` (shared/site.js:1295) is the opposite: strips the editor, inlines CSS/JS and every `assets/` reference as data URIs, and rewrites sibling `.html` links to the live site so a single downloaded file works offline.
...
- Entry `id` doubles as its URL slug (`#/dev/<id>`) and follows the title until the entry is published, then freezes (`dlRetitle`, shared/site.js:1897) so shared links don't break.
- Adding a block type touches four places: `DE_BLOCKS` (shared/site.js:2673) + `DE_NEW` (:2691) for the palette and default shape, `deRenderBlocks()` (shared/site.js:2878) for the editor UI, `dlRenderBlock()` (shared/site.js:2231) for the public render. If it holds media, also the ref-walkers `dlMediaRefs()` (shared/site.js:2145) and `dlExportData()` (shared/site.js:3005), which scan `b.src`, `b.a`, `b.b` and `b.items`.
- Changing entry shape means bumping `DL_SCHEMA` (currently `3`, shared/site.js:1962) and extending `dlMigrate()` (shared/site.js:1964). Migration is idempotent, gated on `e.v`, and runs on every load — old published entries are upgraded in place.
- Routing: project pages handle `#/dev/<id>`; the landing page only redirects legacy `#/<project>/dev/<id>` links (`dlRoute`/`dlLegacyRedirect`, shared/site.js:2430 / :2437).
- Media resolution order is `MEDIA_CACHE` (unpublished, in memory) → `ref.data` (export) → `asset(path)`, in `dlSrc()` (shared/site.js:2077). On error, `dlMediaFallback()` retries from IndexedDB then hides the figure — this is what covers the ~1 min GitHub Pages deploy lag after a publish.

### Theme

`PRESETS` (shared/site.js:1479) hold ~8 seed colours + two font names; `computeVars()` (shared/site.js:1524) derives the full CSS-variable set (shades, `rgba()` tones, dark-mode inversion via luminance) and `applyTheme()` writes them onto `:root`. `portfolio.css` and `shared/theme.css` read the variables — don't hardcode colours in new CSS (R7).

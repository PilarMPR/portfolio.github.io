...
- **Theme presets** — one click each: **Editorial** (the default: warm paper, near-black ink, one deep green accent, a serif for headings), **Plain** (white, no accent, system type), **Notebook** (cream paper, red pen), **Blueprint**, **Midnight** (the dark one), **Rosé**, **Forest**, **Mono**.
...
- **Fonts** — a display face and a body face. The defaults, **Newsreader** and **Source Sans 3**, are linked from every page so they arrive with their italics. **System** is not a webfont at all: it uses whatever UI face the reader's own device has. The rest load from Google Fonts only when you pick them. Display: Newsreader, System, Bricolage Grotesque, Space Grotesk, Syne, Unbounded, Archivo Black, Fraunces. Body: Source Sans 3, System, Space Grotesk, Inter, DM Sans, Work Sans.
- **↺ Reset to default** puts the Editorial preset back.

...
## 2026-09-18 · SESSION · Strip the site back to a plain typographic layout

**Why.** "As plain and least AI as possible, looks wise." The site was carrying
most of the tells of a generated portfolio at once: graph-paper background with
an SVG grain overlay, a yellow highlighter swipe through the name, a handwritten
blue margin note, mono `// KICKER ——` labels, a viewport-filling display name,
index-card project tiles with tape, dog-ears, `FIG.01` strips and `REF-` numbers,
a rotated post-it pull quote, a taped polaroid, a pen-crosshair cursor replacing
the system one, scroll-reveal animations and the Bricolage Grotesque / Space
Grotesk pairing.

**Shipped.** The visual layer of `portfolio.css` was rewritten — `:root`, base,
nav, hero, utils, cards, about, contact, case study, lightbox, the public dev-log
render, project pages and the responsive block. White paper, one ink, hairline
rules, system type, no texture, no motion. The three tool-UI regions (the dark
edit panel, the dev-log editor, the `.ep-*` bits) were spliced through verbatim:
they are deliberately not part of the site's look and nothing there changed.

The theme is a theme change, not a hand-edit: a new `Plain` preset is now
`THEME_DEFAULT`, and `shared/theme.css` was regenerated by running the repo's own
`computeVars()` over it in node, so the published file is byte-identical to what
"Save & publish" would write. Fonts needed a small mechanism: `System` is offered
in both font lists but is not a webfont — `fontStack()` maps it to the device UI
stack and `ensureFont()` skips it. With that, the four Google Fonts `<link>`s came
out of all eight pages and a default page now fetches no font at all.

Three things worth knowing for next time. `#cursor` is a sentinel (R3) so it stays
in the markup and is hidden in CSS rather than removed. `.reveal` and `.reveal.on`
are now identical, so the IntersectionObserver still runs and nothing depends on
it. And `.sh-hero` had to stop being a fixed-height `overflow:hidden` box — with
the gradient scrim gone the case-study title sits under the image instead of over
it, and the old box clipped it away entirely; the height moved onto the `<img>`.

No page content was touched (R1): the only HTML diff is three `<link>` lines
removed per page. Two authored kicker lines carry a literal `//` in their text, so
the layout hides them rather than publishing a code comment as a label — filed as
OPT-22 rather than edited.

`site.js` grew, so all 42 `shared/site.js:NNN` citations in CLAUDE.md were
re-resolved (R11) — and the automatic pass collapsed five distinct symbols onto
shared lines, which the check accepts but a reader would not, so those were bound
back to their real declarations by hand. R7's figures moved 86 → 63 hex literals
and 640 → 415 `var()` uses; the public layer now holds **zero** literals, all 63
being the `:root` fallback and the editor's dark UI. README and SPEC were updated
in the same commit for the new preset and the System font, per the docs rule.

**Verified.** `checks.sh` exits 0. 96 assertions across all eight pages in
Chromium — `window.PAGE` contract, four sentinels (R3), theme applied, system font
resolved, body renders, `edChrome()` idempotent across two calls, `toggleEdit()`
refusing with no token, zero webfont requests, cursor hidden, no looping
animation, no JS errors, no 4xx. Separately: edit mode opens with a token planted,
the panel renders, `buildPublishHTML()` leaks none of the four chrome ids and no
font link, and edit mode closes again. Layout checked at 1440, 768 and 390 — no
horizontal overflow on any page at any of the three.

**Not verified.** `smoke.sh` still skips — no `gjs` in this container — so the
WebKitGTK half is unproven and Chromium stood in for all of it. Safari and Firefox
were not exercised at all, which matters more than usual now that `:has()` and
`aspect-ratio` carry layout. Nothing was published through `ghPublish()`, and no
gesture, drag or tab click was driven — `toggleEdit()` and `edChrome()` were called
directly.

**One thing the user has to do themselves.** A browser that has published from
here holds `pmpr_theme` in `localStorage`, and `loadTheme()` lets it win over
`shared/theme.css`. On that machine the old Notebook colours and fonts will
survive the deploy until the Design tab's **↺ Reset to default** is pressed once
(or the Plain preset is picked). Everyone else gets the plain theme immediately.

**Open / pick up next.**
- OPT-22, OPT-23, OPT-24 filed this session, all inert (R10). OPT-23 is the one
  worth doing soon: the About bio publishes the same paragraph twice and the pull
  quote a third time, and the plain layout no longer hides it behind a post-it.
